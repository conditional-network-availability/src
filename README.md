...
* Currently, due to the limited size of the SRAM available on the target device, TFA executes from external RAM rather than internal SRAM.
While this does not impose functional limitations, it may weaken the device's security against physical attackers.
In particular, a physical attacker could potentially extract the contents of external RAM, for example by performing a cold-boot attack. However, physical attackers are outside the scope of this work.
If protection against such attacks is nevertheless required, one could either use a platform with a larger SRAM capacity or employ a pager mechanism that encrypts and integrity-protects TFA pages that are stored in external DRAM.
