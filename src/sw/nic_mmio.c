...
void nic_write32(uint32_t val, uint64_t address) {
  if(!is_word_in_nic_mmio_space(address)) // {
    // ERROR("Invalid NIC MMIO address: %llx\n", address);
    return;
  // }

  // TODO: Verify MMIO registers for selective enable/disable of ring 2,
  // which is used for our SW!
  ...
  switch (offset) {
    case ENET_TDSR0:
      handle_tx_base_write(0, address, val); return;
    case ENET_TDSR1:
      handle_tx_base_write(1, address, val); return;
    case ENET_TDSR2:
      // For queue 2, we do NOT initialize a shadow ring. This is because
      // we assign ring 2 to the secure world. Whenever the NIC is enabled
      // (via write to ECR, see below), we initialize our SW buffer descriptor
      // ring and reset the write position.
      // handle_tx_base_write(2, address, val)
      // Just do nothing at this point!
      return;
    case ENET_RDSR0:
      handle_rx_base_write(0, address, val); return;
    case ENET_RDSR1:
      handle_rx_base_write(1, address, val); return;
    case ENET_RDSR2:
      handle_rx_base_write(2, address, val); return;
  }
  ...
  switch (offset) {
      ...
      // Initialize SW buffer descriptor ring
      // It is very important that we intialize the ring buffer accordingly.
      // This is taken from the original driver.
      // INFO("Initialize SW TX buffer descriptor ring\n");
      for(uint32_t i=0; i<DESCRING_ELEMENTS_N; i++) {
        // Set the initial values of the buffer descriptor
        struct bufdesc_ex* ele = &state.shadow_tx_descrings[2]->elements[i];

        ele->desc.cbd_bufaddr = htole32(0);
        ele->desc.cbd_sc = htole16((i == DESCRING_ELEMENTS_N-1) ? BD_SC_WRAP : 0);
      }

      // Set the NIC configuration register to use our SW buffer descriptor ring
      // INFO("Set NIC MMIO register to point to SW buffer descriptor ring\n");
      dsb();
      uint32_t* reg = (uint32_t*) (IMX_NIC_BASE + ENET_TDSR2);
      *reg = (uint32_t) ((uint64_t) state.shadow_tx_descrings[SECURE_TX_QUEUE]);
      dsb();

      // Reset TX write position
      // INFO("Reset PTA TX write position\n");
      pta_tx_ring_pos = 0;
...
static uint32_t handle_tx_base_read(uint32_t queue) {
  if(queue == SECURE_TX_QUEUE) // {
    return 0x0; // we do not want to leak this address
  // }
...
uint32_t nic_read32(uint64_t address) {
  if(!is_word_in_nic_mmio_space(address)) // {
