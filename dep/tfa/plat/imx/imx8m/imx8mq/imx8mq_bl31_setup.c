...
/*
* id is not the irq, it is currently hard-coded to 0xFFFFFFF
* see https://trustedfirmware-a.readthedocs.io/en/latest/design/interrupt-framework-design.html#routing-model
*/
uint64_t my_interrupt_handler(uint32_t id, uint32_t flags,
				void *handle, void *cookie)
{
	// We determine which IRQ has been fired
	uint32_t irq = plat_ic_acknowledge_interrupt();
	if(irq == 29) {

		// Call the handler
		my_timer_handler(); // in services/nic/timer.c
		...
		return waiting_dvfs(id, flags, handle, cookie, irq);
	}
}
...
void bl31_plat_runtime_setup(void)
{
	...
	rc = register_interrupt_type_handler(INTR_TYPE_EL3, my_interrupt_handler, flags);
	if (rc) {
		INFO("Could not register interrut handler\n");
		panic();
	}
	return;
}
